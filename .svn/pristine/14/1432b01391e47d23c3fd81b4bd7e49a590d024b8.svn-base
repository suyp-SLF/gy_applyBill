...
public class RecbillListDataPlugin extends AbstractReportListDataPlugin {

    private final static String receivable_table = "cas_recbill";//收款处理
    ...
    public DataSet query(ReportQueryParam reportQueryParam, Object o) throws Throwable {
        ...
        qFilters[0] = new QFilter("billtype.number",QCP.equals,"WBcas_recbill_BT_01");
        qFilters[6] = new QFilter("receivingtype.number",QCP.equals,"101");
        if (StringUtils.isNotBlank(pjtext)){
            qFilters[1] = new QFilter("bfgy_projbd.name", QCP.like, "%"+pjtext+"%");
        }

        if (org != null && StringUtils.isNotBlank(org.getString("number"))){
            qFilters[2] = new QFilter("bfgy_org.number",QCP.equals,org.getString("number"));
        }

        if(StringUtils.isNotBlank(customer)){
            qFilters[3] = new QFilter("bfgy_rncustomer.name",QCP.like,"%"+customer+"%");
        }
        ...
        Map<String, List<DynamicObject>> groupids = receivable_cols.stream().collect(Collectors.groupingBy(i -> i.getString("col_1_01") + "@_@" +
                ...
                (StringUtils.isBlank(i.getString("contractno")) ? "null" : i.getString("contractno")) + "@_@"));
