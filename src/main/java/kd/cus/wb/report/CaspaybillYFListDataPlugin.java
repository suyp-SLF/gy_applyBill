...
public class CaspaybillYFListDataPlugin extends AbstractReportListDataPlugin {
    private final static String receivable_table = "cas_paybill";//财务付款单
    ...
    public DataSet query(ReportQueryParam reportQueryParam, Object o) throws Throwable {
        ...
        String[] receivable_select = {
                ...
                "entry.e_unsettledamt",
                "bfgy_basedcontractdata",
                "auditdate"
        };


        QFilter[] qFilters = new QFilter[7];

        qFilters[0] = new QFilter("billtype.number",QCP.equals,"WBcas_paybill_pur_BT_S_01");
        qFilters[6] = new QFilter("receivingtype.number",QCP.equals,"101");

        if (StringUtils.isNotBlank(pjtext)){
            qFilters[1] = new QFilter("bfgy_basedataxm_wb.name", QCP.like, "%"+pjtext+"%");
        }

        if (org != null && StringUtils.isNotBlank(org.getString("number"))){
            qFilters[2] = new QFilter("bfgy_orgfxdep.number",QCP.equals,org.getString("number"));
        }

        if(StringUtils.isNotBlank(customer)){
            qFilters[3] = new QFilter("providersupplier.name",QCP.like,"%"+customer+"%");
        }
        ...
        Map<String, List<DynamicObject>> groupids = receivable_cols.stream().collect(Collectors.groupingBy(i -> i.getString("col_1_01") + "@_@" +
                ...
                (StringUtils.isBlank(i.getString("bfgy_basedcontractdata"))?"null":i.getString("bfgy_basedcontractdata")) + "@_@"));
            ...
            for(DynamicObject item_i : item.getValue()){
                ...
                if (auditdate != null) {
                    if (auditdate.getTime() < one_year_date.getTime()) {
                        ...
                        col_0_0D = col_0_0D.add(item_i.getBigDecimal("entry.e_unsettledamt"));
                    } else if (auditdate.getTime() < two_year_date.getTime()) {
                        col_1_0.add(item_i.get("id"));
                        col_1_0D = col_1_0D.add(item_i.getBigDecimal("entry.e_unsettledamt"));
                    } else if (auditdate.getTime() < three_year_date.getTime()) {
                        col_2_0.add(item_i.get("id"));
                        col_2_0D = col_2_0D.add(item_i.getBigDecimal("entry.e_unsettledamt"));
                    } else if (auditdate.getTime() < four_year_date.getTime()) {
                        col_3_0.add(item_i.get("id"));
                        col_3_0D = col_3_0D.add(item_i.getBigDecimal("entry.e_unsettledamt"));
                    } else if (auditdate.getTime() < five_year_date.getTime()) {
                        col_4_0.add(item_i.get("id"));
                        col_4_0D = col_4_0D.add(item_i.getBigDecimal("entry.e_unsettledamt"));
                    } else if (auditdate.getTime() >= five_year_date.getTime()) {
                        col_5_0.add(item_i.get("id"));
                        col_5_0D = col_5_0D.add(item_i.getBigDecimal("entry.e_unsettledamt"));
                    }
                }
                col_0.add(col_0_0);
                col_1.add(col_1_0);
